<?php
//
// File generated by ... on the 2024-04-02T09:30:32+0200
// Please do not edit manually
//

/**
 * Classes and menus for combodo-email-synchro (version 3.7.2)
 *
 ...
 */
abstract class MailInboxBase extends _MailInboxBase
{
	public static function Init()
	{
		$aParams = array(			'category' => 'grant_by_profile,application',
			...
			'style' =>  new ormStyle(null, null, null, null, null, 'combodo-email-synchro/images/mailbox.png'),);
		MetaModel::Init_Params($aParams);
		MetaModel::Init_InheritAttributes();
		MetaModel::Init_AddAttribute(new AttributeString("server", array("sql"=>'server', "is_null_allowed"=>false, "default_value"=>'', "allowed_values"=>null, "depends_on"=>array(), "always_load_in_tables"=>false)));
		MetaModel::Init_AddAttribute(new AttributeString("mailbox", array("sql"=>'mailbox', "is_null_allowed"=>true, "default_value"=>'', "allowed_values"=>null, "depends_on"=>array(), "always_load_in_tables"=>false)));
		MetaModel::Init_AddAttribute(new AttributeString("login", array("sql"=>'login', "is_null_allowed"=>false, "default_value"=>'', "allowed_values"=>null, "depends_on"=>array(), "always_load_in_tables"=>false)));
		MetaModel::Init_AddAttribute(new AttributePassword("password", array("sql"=>'password', "is_null_allowed"=>true, "default_value"=>'', "allowed_values"=>null, "depends_on"=>array(), "always_load_in_tables"=>false)));
		MetaModel::Init_AddAttribute(new AttributeEnum("protocol", array("allowed_values"=>new ValueSetEnum("imap,pop3" ), "display_style"=>'list', "sql"=>'protocol', "default_value"=>'imap', "is_null_allowed"=>false, "depends_on"=>array(), "always_load_in_tables"=>false)));
		MetaModel::Init_AddAttribute(new AttributeInteger("port", array("sql"=>'port', "is_null_allowed"=>true, "default_value"=>'', "allowed_values"=>null, "depends_on"=>array(), "always_load_in_tables"=>false)));
		MetaModel::Init_AddAttribute(new AttributeEnum("active", array("allowed_values"=>new ValueSetEnum("no,yes" ), "display_style"=>'list', "sql"=>'active', "default_value"=>'yes', "is_null_allowed"=>false, "depends_on"=>array(), "always_load_in_tables"=>false)));
...
JS
	 ...
	 * @param array $aIgnoredAttachments
	 *
	 ...
	 * @throws \OQLException
	 */
	protected function AddAttachments(Ticket $oTicket, EmailMessage $oEmail, $bNoDuplicates = true, &$aIgnoredAttachments = array())
	{
		...
		foreach($oEmail->aAttachments as $aAttachment)
		{
			...
			if (!in_array($aAttachment['mimeType'], self::$aExcludeAttachments))
			{
				...
				{
					if ($this->IsImage($aAttachment['mimeType']) && class_exists('InlineImage') && $aAttachment['inline'])
					{
	...
	protected function RejectBigAttachment($aAttachment, $oObj)
	{
		...
		$aReplacement = array('content' => $sMessage, 'mimeType' => 'text/plain', 'filename' => 'warning.txt', 'content-id' => $aAttachment['content-id']);
		return $aReplacement;
	}
	...
	public function GetEmailSource()
	{
		...
		switch($sProtocol)
		{
			...
			self::Trace("Protocol: $sProtocol Mail server: $sServer, port: $iPort, login: $sLogin, password: ***, mailbox: $sMailbox, options: /".implode('/', $aImapOptions));
			$oSource = new IMAPEmailSource($sServer, $iPort, $sLogin, $sPwd, $sMailbox, $aImapOptions, $this->Get('target_folder') );
			break;

			case 'pop3':
			$sPop3AuthOption = MetaModel::GetModuleSetting('combodo-email-synchro', 'pop3_auth_option', 'USER');
			self::Trace("Protocol: $sProtocol Mail server: $sServer, port: $iPort, login: $sLogin, password:  ***, auth_option: $sPop3AuthOption");
